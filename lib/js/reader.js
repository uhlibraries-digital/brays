...
function process_metadata(workbook, path) {
  ...
  var out = process_metadata_objects();
  if ( out === false ) return;
...
function rollback(){
  ...
  for ( var k in metadata_array ) {
    ...
    if ( item['dcterms.identifier'] !== '' && item['dcterms.identifier'].indexOf('ark:/') === 0 ) {
...
function delete_identifier(ark){
  var end = settings.destroy_url[settings.destroy_url.length - 1];
  if (end !== '/') { 
