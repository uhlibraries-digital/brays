  ...
  for ( var R = range.s.r; R <= range.e.r; ++R ) {
    ...
    for ( var C = range.s.c; C <= range.e.c; ++C ) {
      ...
      if ( R === 0 ) {
        ...
        if ( worksheet[cell] !== undefined ) {
          item[headers[C]] = worksheet[cell].v;
        }
        else {
          item[headers[C]] = '';
        }
...
function process_metadata_objects(locationpath, subdir) {
  ...
  for ( var k in metadata_array ) {
    ...
    logger.log('Processing object: "' + item['dc.title'] + '"...');
    ...
    for (var i = 0; i < item["_FILES"].length; ++i) {
      ...
      try {
        ...
        logger.log('Failed to move file "' + file["Filename"] + '", ' + err.message);
      }
...
function mint_object(obj, counter) {
  ...
  $.ajax({
    ...
    error: function(data) {
      id = false;
      logger.log('FAILED to mint, trying again...');
    }
  });
  ...
  if ( id === false ) {
    if ( counter === undefined || counter < 5 ) {
      ...
      logger.log("FAILED to mint, I'm giving up");
      return false;
    }
  }
  else {
